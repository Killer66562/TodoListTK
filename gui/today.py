...
class TodayFrame(EventListener):
    def __init__(self, master=None):
        ...
        tk.Label(self.frame, text="今天的活動", font=self.base_font).pack(anchor="w", padx=10, pady=5)
    ...
    def load_today_events(self):
        ...
        with Session(self.engine) as session:
            stmt = select(Activity).where(Activity.starts_at >= start_dt, Activity.starts_at <= end_dt)
